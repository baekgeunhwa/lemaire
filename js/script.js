document.addEventListener(`DOMContentLoaded`, function () {
  ...
  // 마우스휠 이벤트(.header_area 가 휠이 내려갈때는 안보이다가 올라갈때 보이게끔 설정)
  // 클래스 추가 제거로 진행
  ...
  // .sec_2 에 스크롤이 되면 body 전체에 bg 컬러 변경(스크롤 이벤트 offsetTop 값 활용)
  ...
  //작은 그리드에서 햄버거버튼 클릭하면 메인메뉴만 출력 / 햄버거 버튼은 위치값 서로 변경되도록 처리

 const menuBtn = document.querySelector(`#hamburger`);
 const menuMenu = document.querySelector(`.main_menu`);
 ...
 menuBtn.addEventListener(`click`, function(){
  ...
  //`menuBtn 에 active 클래스가 있다`를 변수 안에 저장
